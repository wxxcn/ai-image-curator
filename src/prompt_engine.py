...
class PromptEngine:
    ...
    你是一位精通“通义万相 (Wanx)”生图模型的提示词专家。
    
    [用户任务]: {user_input}
    [目标平台]: {platform}
    [平台风格指南]: {style_guide}
    
    你的任务是利用“思维链”优化用户的简短指令，生成适用于通义万相的高质量**中文提示词**。
    
    步骤 1: 分析用户意图，确定主体、环境、艺术风格（如写实、插画、3D等）和色调。
    步骤 2: 结合[平台风格指南]进行风格化调整。
    步骤 3: 编写最终的中文生图 Prompt。通义万相偏好具体的描述词，如“极致细节”、“电影质感”、“杰作”等。
    ...
    def get_feedback_adjustment_prompt(self, history, feedback):
        ...
        """
        return f"基于此前的生图历史: {history}。用户反馈: {feedback}。请生成一个新的、经过优化的通义万相**中文提示词**。"